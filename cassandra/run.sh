...
docker-compose -f docker-compose-kafka.yml -f docker-compose-redis.yml -f docker-compose-cassandra.yml -f docker-compose-app.yml up -d
...
function clean_up {
    ...
    docker-compose -f docker-compose-kafka.yml -f docker-compose-redis.yml -f docker-compose-cassandra.yml -f docker-compose-app.yml down
    if [ -z "$1" ]
    then
      echo -e "Bye!\n"
    else
...
docker-compose exec redis /opt/redis-stack/bin/redis-cli -a jasonrocks config set notify-keyspace-events KEA 

echo "Number of messages in 'pageviews' stream:"
docker-compose exec redis /opt/redis-stack/bin/redis-cli -a jasonrocks xlen pageviews 

sleep 2
echo -e "\nAdding messages to Redis stream 'mystream':"
docker-compose exec redis /opt/redis-stack/bin/redis-cli -a jasonrocks "xadd" "mystream" "*" "field1" "value11" "field2" "value21" 
docker-compose exec redis /opt/redis-stack/bin/redis-cli -a jasonrocks "xadd" "mystream" "*" "field1" "value12" "field2" "value22"
